...
-->
	<sch:title xmlns="http://www.w3.org/2001/XMLSchema">Nationell metadataprofil 3.1 för Inspire teman (iso19139)</sch:title>
...
-->
	...
	<sch:pattern fpi="[Geodata.se:109]">
		...
		<sch:rule context="/gmd:MD_Metadata//gmd:report/gmd:DQ_DomainConsistency/gmd:result">
			...
			<sch:let name="specification_title" value="//gmd:specification/*/gmd:title/*/text()/lower-case(.)"/>
			<sch:let name="title_count" value="count(//gmd:specification/*/gmd:title/*/text())"/>
...
-->			
			...
			<sch:assert test="($specification_date and $specification_dateType) or not($keywordValue_INS)">[Geodata.se:109d] Resurser som ingår i Inspire skall ha en överensstämmelserapport) - publiceringsdatum för specifikation saknas </sch:assert>
