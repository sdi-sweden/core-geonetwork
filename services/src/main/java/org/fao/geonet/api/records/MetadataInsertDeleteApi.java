...
public class MetadataInsertDeleteApi {
    ...
    private Pair<Integer, String> loadRecord(
        ...
        // If the metadata has the schemaLocation, fix it if required
        Path xslFileFixSchemaLocation = dataMan.getSchemaDir(schema).resolve("set-schemalocation.xsl");
        if (Files.exists(xslFileFixSchemaLocation)) {
            xmlElement = Xml.transform(xmlElement, xslFileFixSchemaLocation);
        }
